// Sistema de Segurança CyberGuard
// Proteção contra ataques DDoS, spam, abuso de recursos e vazamento de dados

console.log('🛡️ CyberGuard Security System carregado');
...
var DataProtection = {
  ...
  maskEmail: function(email) {
    if (!email || !SecurityConfig.enableDataMasking) return email;
    ...
    var maskedDomain = domainParts.length > 1 ?
      domainParts[0].substring(0, 1) + '*'.repeat(domainParts[0].length - 1) + '.' + domainParts.slice(1).join('.') :
      '*'.repeat(domain.length);
    
    return maskedUsername + '@' + maskedDomain;
  },
  
  // Log seguro com mascaramento de dados
  secureLog: function(level, message, data) {
    // Verificar consentimento para logs analíticos
    var canLog = true;
    if (level === 'analytics' && window.privacyConsent) {
      canLog = window.privacyConsent.canUseAnalytics();
    }
    
    if (!canLog) {
      return; // Não fazer log se não há consentimento
    }
    ...
    if (data && this.containsSensitiveData(data)) {
      logEntry.warning = '⚠️ Log contém dados mascarados por segurança';
    }
  ...
  // Verificar se dados contêm informações sensíveis
  containsSensitiveData: function(data) {
    if (!data || typeof data !== 'object') return false;
    
    var sensitiveFields = ['email', 'password', 'userId', 'ip', 'phone', 'cpf'];
  ...
  clearSensitiveStorage: function() {
    try {
      var sensitiveKeys = ['password', 'token', 'auth', 'user_data'];
      sensitiveKeys.forEach(function(key) {
        localStorage.removeItem(key);
        sessionStorage.removeItem(key);
      });
      console.log('✅ Dados sensíveis removidos do storage');
    } catch (error) {
  ...
  checkDataLeaks: function() {
    ...
    try {
      for (var i = 0; i < localStorage.length; i++) {
        ...
        if (key && (key.includes('password') || key.includes('email') || key.includes('auth'))) {
          warnings.push('Dados sensíveis encontrados no localStorage: ' + key);
        }
      }
    } catch (error) {
      // Ignore se não pode acessar localStorage
    }
    
    // Verificar se console está aberto (em desenvolvimento)
    if (typeof console !== 'undefined' && console.clear) {
      var start = performance.now();
      console.clear();
      var end = performance.now();
      if (end - start > 1) {
        warnings.push('DevTools podem estar abertos - dados podem ser visíveis');
      }
    }
  ...
  // Implementar Content Security Policy via JavaScript
  implementCSP: function() {
    ...
    if (!document.querySelector('meta[http-equiv="Content-Security-Policy"]')) {
      ...
      console.log('✅ CSP implementado via JavaScript');
    }
  }
};
...
DataProtection.implementCSP();
DataProtection.clearSensitiveStorage();

// Verificar vazamentos potenciais
setTimeout(function() {
  var warnings = DataProtection.checkDataLeaks();
  if (warnings.length > 0) {
    console.warn('⚠️ Avisos de segurança detectados:', warnings);
  }
}, 2000);
